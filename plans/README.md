...
Two `improve`-skill batches live here, plus one brainstorming-derived bundle under `docs/superpowers/`.

- **Batch 1** (plans 001–006) — generated 2026-07-07 against commit `0e75ab512`; **all DONE / merged** (commit `b74e73fcb`, CI green).
- **Batch 2** (plans 007–031) — generated 2026-07-10, planned against commit `8ee361ea1` (working tree verified at `a8255de56`; cited excerpts confirmed live). **All DONE / merged** to `master` (027 BLOCKED — needs on-device logcat).
- **UI i18n + a11y bundle** — brainstorming-derived (not `improve`), 2026-07-10. Spec `docs/superpowers/specs/2026-07-10-launcher-i18n-a11y-design.md`, plan `docs/superpowers/plans/2026-07-10-launcher-i18n-a11y.md`. Tasks 1–6 **DONE / shipped** to `master` (`14f369e17` a11y, `de8d814e7` i18n; ktlint + `assembleDebug` green). Task 7 (on-device locale/TalkBack/touch-target checks) **PENDING** — cannot run headless. Includes a **follow-on** (2026-07-11): diagnosed a server-port bug (client dials `server_port`/`server_port+world`; server on 43595) and made file-based server-config import the prominent setup path (commit `7f3837901`) — see the plan's "Follow-on" section.
